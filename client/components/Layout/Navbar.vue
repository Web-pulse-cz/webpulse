...
const company = [
  ...
  { label: t('navbar.services'), href: '/sluzby' },
  { label: t('navbar.reference'), href: '/reference' },
...
<template>
  ...
  <header class="fixed z-10 w-full bg-chppGray backdrop-blur-md">
    <nav
      class="max-w-xxl mx-auto flex items-center justify-between px-8 py-6 lg:px-8 lg:py-0 2xl:px-72"
      aria-label="Global"
    >
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-3">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:gap-x-3">
          ...
          <Popover class="hover:text-dark relative py-6 pl-6">
            <PopoverButton class="flex items-center gap-x-1 text-sm/6 text-black">
              <img
                :src="'/static/img/flags/' + locale + '.svg'"
                class="h-6 w-6"
                alt="Locale Flag"
              />
              <ChevronDownIcon class="size-5 flex-none text-black" aria-hidden="true" />
            </PopoverButton>
