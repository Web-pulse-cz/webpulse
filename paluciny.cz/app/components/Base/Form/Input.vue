...
const labelClasses = {
  dark: 'block text-xs lg:text-sm/6 font-medium text-earth',
  light: 'block text-xs lg:text-sm/6 font-medium text-earth-light',
  newsletter: 'block text-xs lg:text-sm/6 font-medium text-white',
};

const inputClasses = {
  light: 'ring-earth-light/30 focus:ring-forest',
  dark: 'ring-earth-light/30 focus:ring-forest',
  newsletter: 'ring-earth-light/50 focus:ring-forest',
};
...
const inputClass = computed(() => {
  return `text-earth mt-2 block w-full rounded-md border-0 py-1.5 lg:py-2 shadow-sm ring-1 ring-inset focus:ring-1 focus:ring-inset text-xs lg:text-sm/6 ${inputClasses[props.variant]}`;
});
</script>
...
<template>
  <div>
    ...
    <Field
      ...
      autocomplete="off"
      :autofocus="false"
      :class="[inputClass, { 'bg-cream-dark': disabled }]"
    />
    <Field
