...
async function saveItem(redirect = true) {
  ...
  loading.value = true;
  await client(
    route.params.id === 'pridat' ? '/api/admin/contract' : '/api/admin/contract/' + route.params.id,
    {
      method: 'POST',
            ...
            <div class="space-y-8">
              ...
              <LayoutContainer v-if="item.type !== 'nda' && item.type !== 'other'">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  />
                  <BaseFormInput
                    v-model="item.vacation_days"
                    label="Dovolená (dní)"
                    name="vacation_days"
                    type="number"
                  />
                  <BaseFormInput
                    v-model="item.notice_period_days"
                    label="Výpovědní lhůta (dní)"
                    name="notice_period_days"
                    type="number"
                  />
                  <BaseFormInput
                    v-model="item.signed_by_employee"
                    label="Podepsáno zaměstnancem"
