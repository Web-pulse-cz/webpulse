...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/review', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      toast.add({
        title: 'Chyba',
        description: 'Nepodařilo se načíst Reference. Zkuste to prosím později.',
        color: 'red',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/review/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      toast.add({
        title: 'Chyba',
        description: 'Nepodařilo se smazat položku Reference.',
        color: 'red',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat recenzi' }]"
      slug="reviews"
    />
    <LayoutContainer>
      <BaseTable
