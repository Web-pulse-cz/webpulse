...
const pageTitle = ref('Fotogalerie');
...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/photo-gallery', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst fotogalerie. Zkuste to prosím později.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/photo-gallery/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat fotogalerii.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat galerii' }]"
