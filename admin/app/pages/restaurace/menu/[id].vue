...
import { DocumentTextIcon, FolderIcon, GlobeAltIcon, NewspaperIcon, TrashIcon } from '@heroicons/vue/24/outline';
...
const pageTitle = ref(
  route.params.id === 'pridat' ? 'Nový jídelní lístek' : 'Detail jídelního lístku',
);
...
async function loadItem() {
  ...
  await client('/api/admin/food/menu/' + route.params.id, {
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst jídelní lístek.',
        severity: 'error',
      });
    })
    .finally(() => {
    ...
    <Form @submit="saveItem">
      ...
      <template v-if="tabs.find((t) => t.current && t.link === '#listek')">
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <button
                  ...
                  :class="{ 'cursor-not-allowed opacity-50': !addSectionId }"
                  @click="addSection"
                >
                  Přidat sekci
                </button>
          ...
          <!-- Sections with items -->
          <LayoutContainer v-for="section in activeSections" :key="section.id">
            <!-- Section header -->
            ...
            <div
              v-if="
                section.items.filter((i) => !i._placeholder || section.items.length === 1)
                  .length === 0 ||
                (section.items.length === 1 && section.items[0]._placeholder)
              "
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="mb-4">
                  <div
                    v-if="menuItem.meal_id"
                    class="flex items-center justify-between rounded-lg bg-indigo-50 px-4 py-2.5 ring-1 ring-indigo-200"
                  >
                    <div>
                      <span class="text-sm font-medium text-indigo-900">{{ menuItem.name }}</span>
                      <span v-if="menuItem.weight" class="ml-2 text-xs text-indigo-600">{{
                        menuItem.weight
                      }}</span>
                    ...
                    <button
                      ...
                      @click="clearMealFromItem(section.id, idx)"
                    >
                      Odebrat
                    </button>
                  </div>
                  ...
                  <div v-else class="relative">
                    ...
                    <input
                      ...
                      :value="mealSearch"
                      @focus="
                        showMealDropdown = section.id + '-' + idx;
                        mealSearch = '';
                      "
                      @input="
                        (e) => {
                          mealSearch = (e.target as HTMLInputElement).value;
                          showMealDropdown = section.id + '-' + idx;
                        }
                      "
                    />
                    ...
                    >
                      <button
                        ...
                        :key="meal.id"
                        type="button"
                        class="flex w-full items-center justify-between px-4 py-2 text-left text-sm transition hover:bg-indigo-50"
                      ...
                      <div
                        v-if="!filteredMeals().length"
                        class="p-3 text-center text-xs text-slate-400"
                      >
                        Žádné jídlo nenalezeno.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                  ...
                  <BaseFormInput
                    ...
                    label="Cena (Kč)"
                    type="number"
                    :name="'price_' + section.id + '_' + idx"
                    :step="1"
                    class="sm:col-span-2"
                  />
                  <BaseFormInput
                    v-model="menuItem.weight"
                    label="Gramáž"
                    :name="'weight_' + section.id + '_' + idx"
                    placeholder="200g"
                    class="sm:col-span-2"
                  />
                  <BaseFormInput
                ...
                <div class="mt-3">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <label
                      ...
                      :class="
                        (menuItem.allergen_ids || []).includes(allergen.id)
