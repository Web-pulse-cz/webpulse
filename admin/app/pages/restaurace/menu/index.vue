...
async function loadItems() {
	...
	await client<{ id: number }>('/api/admin/food/menu', {
		method: 'GET',
...
async function deleteItem(id: number) {
	...
	await client<{ id: number }>('/api/admin/food/menu/' + id, {
		method: 'DELETE',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
		...
		.catch(() => {
			...
			$toast.show({
				summary: 'Chyba',
				detail: 'Nepodařilo se smazat menu.',
				severity: 'error',
			});
		})
		.finally(() => {
...
<template>
	<div>
		<LayoutHeader
			...
			:actions="[{ type: 'add', text: 'Přidat menu' }]"
			slug="menus"
		/>
		<BaseTable
			:items="items"
