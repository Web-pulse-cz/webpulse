...
async function loadItem() {
	...
	await client('/api/admin/food/menu-section/' + route.params.id, {
		...
		breadcrumbs.value[2] = { name: r.name, link: '/restaurace/menu/sekce/' + r.id, current: true };
...
async function saveItem(redirect = true) {
	const client = useSanctumClient();
	loading.value = true;
	await client(route.params.id === 'pridat' ? '/api/admin/food/menu-section' : '/api/admin/food/menu-section/' + route.params.id, {
	...
	}).catch(() => { $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit sekci.', severity: 'error' }); })
	.finally(() => { loading.value = false; });
}
...
<template>
	<div class="space-y-6 pb-20">
		<LayoutHeader :title="pageTitle" :breadcrumbs="breadcrumbs" :actions="[{ type: 'save' }, { type: 'save-and-stay' }]" slug="menus" @save="saveItem" />
				...
				<div class="col-span-1 lg:col-span-9">
					<LayoutContainer>
						...
						<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
							<BaseFormInput v-model="item.name" label="Název" name="name" rules="required" placeholder="Např. Předkrmy, Polévky, Dezerty..." />
							<BaseFormInput v-model="item.position" label="Pořadí" type="number" name="position" />
							<BaseFormInput v-model="item.description" label="Popis" name="description" class="col-span-full" />
						</div>
					</LayoutContainer>
				</div>
