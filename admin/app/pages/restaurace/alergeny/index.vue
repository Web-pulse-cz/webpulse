...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/food/allergen', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst alergeny. Zkuste to prosím později.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/food/allergen/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat položku alergenu.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat alergen' }]"
      slug="allergens"
    />
    <BaseTable
      :items="items"
