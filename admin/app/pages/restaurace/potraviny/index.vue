...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/food/foodstuff', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst potraviny. Zkuste to prosím později.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/food/foodstuff/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat potravinu.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat potravinu' }]"
      slug="foodstuffs"
    />
    <BaseTable
      :items="items"
