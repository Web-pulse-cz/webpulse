...
const pageTitle = ref('Kategorie potravin');
...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/food/foodstuff/category', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst kategorie potravin. Zkuste to prosím později.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/food/foodstuff/category/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat položku kategorie potravin.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat kategorii potravin' }]"
      slug="foodstuffs"
    />
    <BaseTable
      :items="items"
      ...
      :loading="loading"
      :error="error"
      singular="Kategorie potravin"
      plural="Kategorie potravin"
      :query="tableQuery"
      slug="foodstuffs"
