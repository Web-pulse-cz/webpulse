...
const breadcrumbs = ref([
	...
	},
	{
		...
		link: '/restaurace/recepty/kategorie/pridat',
		current: true,
	},
]);
...
async function loadItem() {
	...
	await client('/api/admin/food/recipe/category/' + route.params.id, {
		method: 'GET',
		headers: {
		...
		.then((response) => {
			...
			breadcrumbs.value.push({
				name: pageTitle.value,
				link: '/restaurace/recepty/kategorie/' + route.params.id,
				current: true,
			});
		...
		.catch(() => {
			...
			router.push('/restaurace/recepty/kategorie');
		})
		.finally(() => {
			loading.value = false;
		});
...
async function saveItem(redirect = true as boolean) {
	...
	await client(
		route.params.id === 'pridat'
			? '/api/admin/food/recipe/category'
			: '/api/admin/food/recipe/category/' + route.params.id,
		{
			method: 'POST',
		...
		.then((response) => {
			...
			if (!redirect && route.params.id === 'pridat') {
				router.push('/restaurace/recepty/kategorie/' + response.id);
			} else if (redirect) {
				router.push('/restaurace/recepty/kategorie');
			} else {
				loadItem();
		...
		.catch(() => {
			...
			$toast.show({
				summary: 'Chyba',
				detail: 'Nepodařilo se uložit kategorii. Zkontrolujte, že máte vyplněna všechna povinná pole.',
				severity: 'error',
			});
		})
		.finally(() => {
