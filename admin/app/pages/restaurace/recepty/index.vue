...
async function loadItems() {
  ...
  await client<{ id: number }>('/api/admin/food/recipe', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst recepty. Zkuste to prosím později.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/food/recipe/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat recept.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat recept' }]"
      slug="recipes"
    />
    <BaseTable
      :items="items"
