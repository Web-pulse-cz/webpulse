...
const editingBoard = ref({ id: null, name: '', color: '#6366f1', is_completed: false, position: 0, sites: [] as number[] });
...
async function loadBoards() {
	...
	}).then((r) => { boards.value = r; })
	.finally(() => { loading.value = false; });
}

function openNew() {
	editingBoard.value = { id: null, name: '', color: '#6366f1', is_completed: false, position: 0, sites: [] };
	showForm.value = true;
}
...
function openEdit(board: any) {
	editingBoard.value = {
		...
		is_completed: board.is_completed, position: board.position,
		sites: board.sites || [],
	};
	showForm.value = true;
}
...
async function saveBoard() {
	...
	const client = useSanctumClient();
	const url = editingBoard.value.id ? '/api/admin/task-board/' + editingBoard.value.id : '/api/admin/task-board';
	await client(url, {
		method: 'POST',
			...
			>
				<div class="mb-3 flex items-center justify-between">
					...
					<div class="flex items-center gap-1">
						<span v-if="board.is_completed" class="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700">
							<CheckCircleIcon class="mr-0.5 inline size-3" /> Dokončený
						</span>
						<button type="button" class="rounded-lg p-1 text-slate-300 hover:bg-red-50 hover:text-red-500 transition" @click.stop="deleteBoard(board.id)">
			...
			<Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
				<div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showForm = false">
					<div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
						<h3 class="mb-4 text-lg font-bold text-slate-900">{{ editingBoard.id ? 'Upravit board' : 'Nový board' }}</h3>
						<div class="space-y-4">
							<BaseFormInput v-model="editingBoard.name" label="Název" name="board_name" rules="required" />
							<div class="grid grid-cols-2 gap-4">
								<BaseFormColorPicker v-model="editingBoard.color" label="Barva" name="board_color" />
								<BaseFormInput v-model="editingBoard.position" label="Pořadí" type="number" name="board_position" />
							</div>
							<BaseFormCheckbox v-model="editingBoard.is_completed" label="Board označuje úkoly jako dokončené" name="board_done" />
							<div class="flex justify-end gap-3 pt-2">
								...
								<button type="button" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500" @click="saveBoard">Uložit</button>
							</div>
						</div>
					</div>
				</div>
