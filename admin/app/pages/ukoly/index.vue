...
async function startTaskTimer() {
  ...
  await client('/api/admin/time-entry/timer/start', {
    ...
    .then(() => {
      ...
      $toast.show({
        summary: 'Timer spuštěn',
        detail: 'Přejděte na Sledování času pro zobrazení.',
        severity: 'success',
      });
    })
    .catch(() => {
      ...
      :inline-filters="[
        {
          ...
          label: 'Projekt',
          type: 'select',
          modelValue: filterProject,
          options: [{ value: '', name: 'Všechny projekty' }, ...projects],
        },
        {
          key: 'assignee_id',
          label: 'Přiřazeno',
          type: 'select',
          modelValue: filterAssignee,
          options: [{ value: '', name: 'Všichni' }, ...users],
        },
      ]"
      @apply-filters="
        (p) => {
          if (p.key === 'project_id') filterProject = p.value || null;
          else if (p.key === 'assignee_id') filterAssignee = p.value || null;
          else if (p.key === '_apply') applyFilters();
        }
      "
    />
      ...
      <div class="flex flex-col gap-3 sm:flex-row">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <BaseFormSelect
            ...
            label=""
            name="nt_project"
            :options="[{ value: null, name: 'Bez projektu' }, ...projects]"
            class="w-full sm:w-36"
          />
          <BaseFormSelect
            ...
            <div class="flex-1 space-y-5 overflow-y-auto px-6 py-5">
              ...
              <div v-if="selectedTask.time_entries?.length" class="border-t border-slate-100 pt-4">
                ...
                <div class="space-y-1">
                  ...
                  >
                    <span
                      >{{ formatSeconds(te.seconds) }} — {{ te.description || '—' }} ({{
                        te.date
                      }})</span
                    >
