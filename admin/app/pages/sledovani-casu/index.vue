...
const groupedEntries = computed(() => {
  const groups: Record<
    string,
    { date: string; dateFormatted: string; items: Record<string, any> }
  > = {};
  ...
  for (const entry of entries.value as any[]) {
    ...
    const key =
      (entry.description || '—') + '|' + (entry.project_id || '') + '|' + (entry.task_id || '');
...
async function startTimer() {
  if (!timerDescription.value && !timerTaskId.value) {
    $toast.show({
      summary: 'Info',
      detail: 'Zadejte popis nebo vyberte úkol.',
      severity: 'warning',
    });
    return;
  }
...
<template>
  <div class="space-y-6 pb-20">
    ...
    <template v-if="tabs.find((t) => t.current && t.link === '#prehled')">
      <div
        v-if="!groupedEntries.length && !loading"
            ...
            >
              <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    v-if="group.task_code"
                    class="rounded-full bg-indigo-50 px-2 py-0.5 font-mono text-[10px] font-bold text-indigo-600"
                    >{{ group.task_code }}</span
                  >
                  <span
                    v-if="group.project_name"
                ...
                <div class="flex flex-wrap items-center gap-3">
                  ...
                  <span
                    v-if="group.totalPriceWithoutVat > 0"
                    class="text-xs tabular-nums text-slate-500"
                  >
                    {{ formatNumber(group.totalPriceWithoutVat) }} {{ group.currency_symbol }}
                    <span v-if="group.totalVat > 0" class="text-slate-400">
                      + {{ formatNumber(group.totalVat) }} DPH =
                      <span class="font-medium text-slate-700"
                        >{{ formatNumber(group.totalPriceWithVat) }}
                        {{ group.currency_symbol }}</span
                      ></span
                    >
                  </span>
                  <button
                    type="button"
                    class="rounded-lg p-1.5 text-slate-400 transition hover:bg-emerald-50 hover:text-emerald-600"
                    title="Spustit znovu"
                    @click="restartEntry(group.entries[0])"
                  >
              ...
              <div v-if="group.entries.length > 1" class="mt-2 border-t border-slate-100 pt-2">
                <div
                  ...
                  :key="entry.id"
                  class="flex items-center justify-between py-1 text-xs text-slate-500"
                >
      ...
      <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <BaseFormSelect
            ...
            label="Projekt"
            name="f_project"
            :options="[{ value: null, name: 'Všechny' }, ...projects]"
          />
          <BaseFormSelect
            v-model="filters.task_id"
            label="Úkol"
            name="f_task"
            :options="[{ value: null, name: 'Všechny' }, ...tasks]"
          />
          <BaseFormSelect
            v-model="filters.user_id"
            label="Uživatel"
            name="f_user"
            :options="[{ value: null, name: 'Všichni' }, ...users]"
          />
          <BaseFormInput
        ...
        <div class="mt-4 flex flex-wrap gap-3">
          <button
            type="button"
            class="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500"
            @click="loadEntries()"
          >
          ...
          <button
            type="button"
            class="flex items-center gap-2 rounded-lg bg-slate-700 px-4 py-2 text-sm font-medium text-white hover:bg-slate-600"
            @click="exportPdf"
          >
