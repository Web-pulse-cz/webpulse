...
<template>
  <div class="space-y-6 pb-20">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div v-if="runningEntry" class="text-sm text-slate-500">
            <span v-if="runningEntry.project_name" class="font-medium">{{
        ...
        <div class="flex items-center gap-3">
          <template v-if="!runningEntry">
            ...
            <button
              type="button"
              class="rounded-lg bg-green-600 px-6 py-3 text-sm font-medium text-white transition hover:bg-green-500"
              @click="startTimer"
            >
              Start
            </button>
          ...
          <button
            v-else
            type="button"
            class="rounded-lg bg-red-600 px-6 py-3 text-sm font-medium text-white transition hover:bg-red-500"
            @click="stopTimer"
          >
            Stop
          </button>
    ...
    <!-- Table -->
    <LayoutContainer>
      <div v-if="!items.length && !loading" class="py-12 text-center text-sm text-slate-400">
        Žádné záznamy odpovídající filtru.
      </div>
      <table v-else class="w-full text-sm">
        <thead>
          <tr
            class="border-b border-slate-200 text-left text-xs font-bold uppercase tracking-wider text-slate-400"
          >
            <th class="px-2 py-3">Datum</th>
            <th class="px-2 py-3">Projekt</th>
            <th class="px-2 py-3">Úkol</th>
            <th class="px-2 py-3">Uživatel</th>
            <th class="px-2 py-3">Popis</th>
            <th class="px-2 py-3 text-right">Hodiny</th>
            <th class="px-2 py-3 text-right">Sazba</th>
            <th class="px-2 py-3"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          <tr v-for="entry in items" :key="entry.id" class="transition hover:bg-slate-50">
            <td class="px-2 py-3 text-slate-600">{{ entry.date }}</td>
            <td class="px-2 py-3">{{ entry.project_name || '—' }}</td>
            <td class="px-2 py-3">
              <span v-if="entry.task_code" class="font-mono text-xs text-indigo-500">{{
                entry.task_code
              }}</span>
              <span v-if="entry.task_name" class="ml-1">{{ entry.task_name }}</span>
              <span v-if="!entry.task_code && !entry.task_name">—</span>
            </td>
            <td class="px-2 py-3 text-slate-500">{{ entry.user_name || '—' }}</td>
            <td class="px-2 py-3 text-slate-500">{{ entry.description || '—' }}</td>
            <td class="px-2 py-3 text-right font-medium">
              {{ entry.hours }}h
              <span
                v-if="entry.is_running"
                class="ml-1 rounded-full bg-green-100 px-1.5 py-0.5 text-[10px] font-medium text-green-700"
                >Běží</span
              >
            </td>
            <td class="px-2 py-3 text-right text-slate-400">{{ entry.hourly_rate || '—' }}</td>
