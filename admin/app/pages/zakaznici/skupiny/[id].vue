...
const item = ref({
	...
	discount_type: null as string | null, discount_value: 0,
	discount_currency_id: null, position: 0, sites: [] as number[],
});
...
async function saveItem(redirect = true) {
	const client = useSanctumClient();
	loading.value = true;
	await client(route.params.id === 'pridat' ? '/api/admin/customer/group' : '/api/admin/customer/group/' + route.params.id, {
	...
	}).catch(() => { $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit skupinu.', severity: 'error' }); })
	.finally(() => { loading.value = false; });
}
...
<template>
	<div class="space-y-6 pb-20">
		<LayoutHeader :title="pageTitle" :breadcrumbs="breadcrumbs" :actions="[{ type: 'save' }, { type: 'save-and-stay' }]" slug="customers" @save="saveItem" />
		<Form @submit="saveItem">
			<div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
				<div class="col-span-1 lg:col-span-9">
					<LayoutContainer>
						<div class="mb-6 flex items-center gap-3">
							<div class="flex size-8 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600"><UsersIcon class="size-5" /></div>
					...
					<LayoutContainer class="mt-8">
						...
						<div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
							...
							<BaseFormInput v-model="item.discount_value" label="Hodnota slevy" type="number" name="discount_value" :step="0.01" />
							<BaseFormSelect v-if="item.discount_type === 'fixed'" v-model="item.discount_currency_id" label="Měna" name="discount_currency_id" :options="currencyStore.currenciesOptions" />
						</div>
					</LayoutContainer>
				</div>
