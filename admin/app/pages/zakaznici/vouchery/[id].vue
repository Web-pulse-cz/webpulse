...
async function saveItem(redirect = true) {
  ...
  loading.value = true;
  await client(
    route.params.id === 'pridat' ? '/api/admin/voucher' : '/api/admin/voucher/' + route.params.id,
    {
      method: 'POST',
    ...
    .catch(() => {
      $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit voucher.', severity: 'error' });
    })
    .finally(() => {
      loading.value = false;
    });
        ...
        <div class="col-span-1 space-y-8 lg:col-span-9">
          <LayoutContainer>
            <div class="mb-6 flex items-center gap-3">
              ...
              >
                <DocumentCurrencyEuroIcon class="size-5" />
              </div>
          ...
          <LayoutContainer>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
              ...
              <BaseFormInput
                ...
                :label="item.discount_type === 'percentage' ? 'Sleva (%)' : 'Sleva (částka)'"
                type="number"
                name="discount_value"
                :step="0.01"
              />
              ...
              />
              <BaseFormInput
                v-model="item.min_order_value"
                label="Minimální hodnota objednávky"
                type="number"
          ...
          <LayoutContainer>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              />
              <BaseFormInput
                v-model="item.max_uses"
                label="Max. počet použití celkem"
                type="number"
                name="max_uses"
                placeholder="Neomezeno"
              />
              <BaseFormInput
                v-model="item.max_uses_per_customer"
                label="Max. použití na zákazníka"
                type="number"
            ...
            <div
              class="mt-4 flex items-center gap-3 rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200"
            >
          ...
          <LayoutContainer>
            ...
            <p class="mb-4 text-sm text-slate-500">
              Pokud nevyberete žádného zákazníka, voucher bude platit pro všechny.
            </p>
            <div class="max-h-60 space-y-1 overflow-y-auto rounded-xl border border-slate-200 p-3">
              ...
              >
                <input
                  ...
                  @change="
                    item.customers.includes(customer.id)
                      ? (item.customers = item.customers.filter((id) => id !== customer.id))
                      : item.customers.push(customer.id)
                  "
                />
                <div>
                  <span class="text-sm font-medium text-slate-900">{{ customer.full_name }}</span>
                  <span v-if="customer.email" class="ml-2 text-xs text-slate-400">{{
                    customer.email
                  }}</span>
                </div>
              </label>
              <div v-if="!allCustomers.length" class="py-4 text-center text-sm text-slate-400">
                Žádní zákazníci.
              </div>
            </div>
          </LayoutContainer>
