...
const item = ref({
	...
	date_of_birth: '', gender: null, street: '', city: '', zip: '', country_id: null,
	company_name: '', ico: '', dic: '',
	total_spent: 0, credit_balance: 0, currency_id: null,
	rating: null, customer_group_id: null, status: 'active',
	note: '', sites: [] as number[], vouchers: [] as any[],
});
...
async function loadItem() {
	...
	await client('/api/admin/customer/' + route.params.id, {
		...
		breadcrumbs.value[1] = { name: pageTitle.value, link: '/zakaznici/' + route.params.id, current: true };
...
async function saveItem(redirect = true) {
	const client = useSanctumClient();
	loading.value = true;
	await client(route.params.id === 'pridat' ? '/api/admin/customer' : '/api/admin/customer/' + route.params.id, {
		method: 'POST', body: JSON.stringify(item.value),
		headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
	}).then((r) => {
		$toast.show({ summary: 'Hotovo', detail: route.params.id === 'pridat' ? 'Zákazník vytvořen.' : 'Zákazník upraven.', severity: 'success' });
		if (!redirect && route.params.id === 'pridat') router.push('/zakaznici/' + r.id);
		else if (redirect) router.push('/zakaznici');
		else loadItem();
	}).catch(() => { $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit zákazníka.', severity: 'error' }); })
	.finally(() => { loading.value = false; });
}
...
<template>
	<div class="space-y-6 pb-20">
		<LayoutHeader :title="pageTitle" :breadcrumbs="breadcrumbs" :actions="[{ type: 'save' }, { type: 'save-and-stay' }]" :modify-bottom="false" slug="customers" @save="saveItem" />
		...
		<Form @submit="saveItem">
			...
			<template v-if="tabs.find((t) => t.current && t.link === '#osobni')">
				<div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
					<div class="col-span-1 space-y-8 lg:col-span-9">
						<LayoutContainer>
							<div class="mb-6 flex items-center gap-3">
								<div class="flex size-8 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600"><UserIcon class="size-5" /></div>
					...
					<div class="col-span-1 space-y-6 lg:sticky lg:top-24 lg:col-span-3">
						...
						<LayoutContainer class="!py-6">
							<LayoutTitle class="text-sm uppercase tracking-widest text-slate-400">Skupina</LayoutTitle>
							<div class="mt-4"><BaseFormSelect v-model="item.customer_group_id" label="" name="group" :options="groups" /></div>
						</LayoutContainer>
						<LayoutContainer class="!py-6">
							<LayoutTitle class="text-sm uppercase tracking-widest text-slate-400">Hodnocení</LayoutTitle>
							<div class="mt-3 flex gap-1">
								<button v-for="star in 5" :key="star" type="button" @click="item.rating = item.rating === star ? null : star">
									<StarIcon :class="['size-7 transition', star <= (item.rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-300']" />
								</button>
							</div>
						</LayoutContainer>
						<LayoutContainer class="!py-6">
			...
			<template v-if="tabs.find((t) => t.current && t.link === '#finance')">
				<LayoutContainer>
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
						<BaseFormInput v-model="item.total_spent" label="Celková útrata" type="number" name="total_spent" :step="0.01" />
						<BaseFormInput v-model="item.credit_balance" label="Kreditní zůstatek" type="number" name="credit_balance" :step="0.01" />
			...
			<template v-if="tabs.find((t) => t.current && t.link === '#vouchery')">
				<LayoutContainer>
					...
					<div v-else class="divide-y divide-slate-100">
						<div v-for="v in item.vouchers" :key="v.id" class="flex items-center justify-between py-3">
							<div>
