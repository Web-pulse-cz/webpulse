...
const tableQuery = ref({ search: null as string | null, paginate: 12, page: 1, orderBy: 'last_name', orderWay: 'asc' });
...
async function loadItems() {
	...
	.catch(() => { error.value = true; $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst zákazníky.', severity: 'error' }); })
...
<template>
	<div>
		<LayoutHeader :title="pageTitle" :breadcrumbs="breadcrumbs" :actions="[{ type: 'add', text: 'Přidat zákazníka' }]" slug="customers" />
		<BaseTable
			:items="items"
			:columns="[
				{ key: 'id', name: 'ID', type: 'text', width: 60, hidden: false, sortable: true },
				{ key: 'last_name', name: 'Příjmení', type: 'text', width: 120, hidden: false, sortable: true },
				{ key: 'first_name', name: 'Jméno', type: 'text', width: 120, hidden: false, sortable: true },
				{ key: 'email', name: 'E-mail', type: 'text', width: 150, hidden: true, sortable: true },
				{ key: 'total_spent', name: 'Útrata', type: 'number', width: 100, hidden: true, sortable: true },
				{ key: 'rating', name: 'Hodnocení', type: 'stars', width: 100, hidden: false, sortable: true },
				{ key: 'group_name', name: 'Skupina', type: 'text', width: 100, hidden: true, sortable: false },
				{ key: 'status', name: 'Stav', type: 'text', width: 80, hidden: false, sortable: true },
			]"
			:actions="[{ type: 'edit' }, { type: 'delete' }]"
			:loading="loading" :error="error"
			singular="Zákazník" plural="Zákazníci"
