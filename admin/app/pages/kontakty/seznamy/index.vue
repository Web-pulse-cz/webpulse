...
const pageTitle = ref('Seznamy kontaktů');
...
async function deleteItem(id: number) {
  ...
  await client<{ id: number }>('/api/admin/contact/list/' + id, {
    method: 'DELETE',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    ...
    .catch(() => {
      ...
      toast.add({
        title: 'Chyba',
        description: 'Nepodařilo se smazat položku seznamu kontaktů.',
        color: 'red',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat seznam kontaktů' }]"
      slug="contacts"
    />
    <LayoutContainer>
      <BaseTable
        ...
        :loading="loading"
        :error="error"
        singular="Seznamy kontaktů"
        plural="Seznam kontaktů"
        :query="tableQuery"
        slug="contacts"
