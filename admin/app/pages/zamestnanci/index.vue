...
const tableQuery = ref({
	...
	paginate: 12, page: 1, orderBy: 'last_name', orderWay: 'asc',
});
...
async function loadItems() {
	...
	.catch(() => { error.value = true; $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst zaměstnance.', severity: 'error' }); })
...
<template>
	<div>
		<LayoutHeader :title="pageTitle" :breadcrumbs="breadcrumbs" :actions="[{ type: 'add', text: 'Přidat zaměstnance' }]" slug="employees" />
		<BaseTable
			:items="items"
			:columns="[
				...
				{ key: 'date_hired', name: 'Nástup', type: 'text', width: 100, hidden: true, sortable: true },
			]"
			:actions="[{ type: 'edit' }, { type: 'delete' }]"
			:loading="loading" :error="error"
			singular="Zaměstnanec" plural="Zaměstnanci"
