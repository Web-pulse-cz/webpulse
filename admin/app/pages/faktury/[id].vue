...
async function saveItem(redirect = true) {
	...
	await client(
		...
		.then((response) => {
			$toast.show({
				summary: 'Hotovo',
				detail: route.params.id === 'pridat' ? 'Faktura byla úspěšně vytvořena.' : 'Faktura byla úspěšně upravena.',
				severity: 'success',
			});
					...
					<div class="col-span-1 space-y-8 lg:col-span-9">
						...
						<LayoutContainer>
							...
							<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
								...
								<BaseFormTextarea v-model="item.footer_note" label="Patička" name="footer_note" rows="3" />
							</div>
						</LayoutContainer>
					</div>
