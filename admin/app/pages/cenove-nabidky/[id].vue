...
async function loadItem() {
	...
	await client('/api/admin/price-offer/' + route.params.id, {
		...
		.then((response) => {
			item.value = {
				...response,
				sites: Array.isArray(response.sites) ? response.sites.map((s: any) => typeof s === 'object' ? s.id : s) : [],
...
async function loadClients() {
	...
	await client('/api/admin/client', {
		...
		.then((response) => {
			...
			clients.value = Array.isArray(data) ? data.map((c: any) => ({ value: c.id, name: c.name })) : [];
		})
		.catch(() => {});
}
...
async function loadProjects() {
	...
	await client('/api/admin/project', {
		...
		.then((response) => {
			const data = response?.data || response;
			projects.value = Array.isArray(data) ? data.map((p: any) => ({ value: p.id, name: p.name })) : [];
		})
		.catch(() => {});
}
					...
					<div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
						...
						<div class="mt-4 space-y-3">
							<label
								...
								:class="acceptDocumentType === 'invoice' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'"
							>
							...
							<label
								class="flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition"
								:class="acceptDocumentType === 'proforma' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'"
							>
