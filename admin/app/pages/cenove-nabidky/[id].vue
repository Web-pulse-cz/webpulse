...
async function loadItem() {
	...
	await client('/api/admin/price-offer/' + route.params.id, {
		...
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst cenovou nabídku.', severity: 'error' });
		})
		.finally(() => {
			loading.value = false;
		});
...
async function saveItem(redirect = true) {
	...
	await client(
		route.params.id === 'pridat' ? '/api/admin/price-offer' : '/api/admin/price-offer/' + route.params.id,
		{
			method: 'POST',
...
async function acceptOffer() {
	...
	await client('/api/admin/price-offer/' + route.params.id + '/accept', {
		...
		.then((response) => {
			...
			$toast.show({ summary: 'Hotovo', detail: 'Nabídka byla přijata a faktura vytvořena.', severity: 'success' });
			loadItem();
		})
		.catch(() => {
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se přijmout nabídku.', severity: 'error' });
		})
		.finally(() => {
			loading.value = false;
		});
...
async function rejectOffer() {
	...
	await client('/api/admin/price-offer/' + route.params.id + '/reject', {
		...
		.catch(() => {
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se zamítnout nabídku.', severity: 'error' });
		})
		.finally(() => {
			loading.value = false;
		});
...
<template>
	<div class="space-y-6 pb-20">
				...
				<div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
					<div class="col-span-1 space-y-8 lg:col-span-9">
						<LayoutContainer>
							...
							<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
								<BaseFormInput v-model="item.title" label="Název nabídky" type="text" name="title" class="col-span-full" />
								<BaseFormSelect v-model="item.client_id" label="Klient" name="client_id" :options="clients" />
								<BaseFormSelect v-model="item.project_id" label="Projekt" name="project_id" :options="projects" />
								...
								<BaseFormInput v-model="item.valid_to" label="Platnost do" type="date" name="valid_to" />
							</div>
						</LayoutContainer>
						...
						<LayoutContainer>
							...
							<div class="grid grid-cols-1 gap-6">
								<BaseFormTextarea v-model="item.introduction" label="Úvod" name="introduction" rows="3" />
								<BaseFormTextarea v-model="item.terms" label="Obchodní podmínky" name="terms" rows="3" />
					...
					<div class="col-span-1 space-y-6 lg:sticky lg:top-24 lg:col-span-3">
						<LayoutContainer class="!py-6">
							...
							<div class="mt-4">
								<BaseFormSelect v-model="item.status" label="" name="status" :options="statusOptions" />
							</div>
						</LayoutContainer>
						...
						<LayoutContainer v-if="item.id && route.params.id !== 'pridat'" class="!py-6">
							<LayoutTitle class="text-sm uppercase tracking-widest text-slate-400">Akce</LayoutTitle>
							<div class="mt-4 space-y-2">
								<button
									v-if="item.status === 'draft' || item.status === 'sent'"
									type="button"
									class="w-full rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-500 transition"
								...
								<button
									v-if="item.status === 'draft' || item.status === 'sent'"
									type="button"
									class="w-full rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-500 transition"
									@click="rejectOffer"
								>
									Zamítnout
								</button>
								<button
									type="button"
									class="w-full rounded-lg bg-slate-600 px-4 py-2 text-sm font-medium text-white hover:bg-slate-500 transition"
									@click="downloadPdf"
								>
									Stáhnout PDF
								</button>
								<NuxtLink
									...
									class="block w-full rounded-lg bg-indigo-600 px-4 py-2 text-center text-sm font-medium text-white hover:bg-indigo-500 transition"
								>
									Zobrazit fakturu
								</NuxtLink>
							</div>
						...
						<LayoutContainer v-if="item.total_with_vat" class="!py-6">
							<LayoutTitle class="text-sm uppercase tracking-widest text-slate-400">Celkem</LayoutTitle>
					...
					<div v-else class="space-y-4">
						...
						>
							<div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
								<BaseFormInput v-model="lineItem.name" label="Název" name="item_name" class="sm:col-span-3" />
								<BaseFormInput v-model="lineItem.description" label="Popis" name="item_desc" class="sm:col-span-3" />
								<BaseFormInput v-model="lineItem.quantity" label="Množství" type="number" name="item_qty" :step="0.01" class="sm:col-span-1" />
								<BaseFormInput v-model="lineItem.unit_name" label="Jedn." name="item_unit" class="sm:col-span-1" />
								<BaseFormInput v-model="lineItem.unit_price_without_vat" label="Cena/ks" type="number" name="item_price" :step="0.01" class="sm:col-span-2" />
								<BaseFormInput v-model="lineItem.vat_rate" label="DPH %" type="number" name="item_vat" class="sm:col-span-1" />
		...
		<Teleport to="body">
			...
			>
				<div v-if="showAcceptDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showAcceptDialog = false">
					<div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
						<h3 class="text-lg font-bold text-slate-900">Přijmout nabídku</h3>
						<p class="mt-2 text-sm text-slate-500">Vyberte typ dokladu, který se vytvoří z této nabídky:</p>
						<div class="mt-4 space-y-3">
							<label class="flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition" :class="acceptDocumentType === 'invoice' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'">
								<input v-model="acceptDocumentType" type="radio" value="invoice" class="text-indigo-600" />
								<span class="text-sm font-medium">Faktura</span>
							</label>
							<label class="flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition" :class="acceptDocumentType === 'proforma' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'">
								<input v-model="acceptDocumentType" type="radio" value="proforma" class="text-indigo-600" />
								<span class="text-sm font-medium">Proforma faktura</span>
							</label>
						</div>
						<div class="mt-6 flex justify-end gap-3">
							<button type="button" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 transition" @click="showAcceptDialog = false">Zrušit</button>
							<button type="button" class="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-500 transition" @click="acceptOffer">Přijmout</button>
						</div>
					</div>
				</div>
			</Transition>
