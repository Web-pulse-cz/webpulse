...
async function loadApartments() {
	...
	await client('/api/admin/apartment', {
		...
		.then((response: any) => {
			apartments.value = (Array.isArray(response) ? response : response.data || [])
				.map((a: any) => ({ value: a.id, name: a.name || a.code }));
		})
		.catch(() => {
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst apartmány.', severity: 'error' });
		});
}
...
async function loadItem() {
	...
	await client('/api/admin/apartment/reservation/' + route.params.id, {
		...
		.then((response: any) => {
			...
			breadcrumbs.value.push({ name: pageTitle.value, link: '/ubytovani/rezervace/' + route.params.id, current: true });
		})
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst rezervaci.', severity: 'error' });
...
async function saveItem(redirect = true as boolean) {
	...
	await client(
		route.params.id === 'pridat' ? '/api/admin/apartment/reservation' : '/api/admin/apartment/reservation/' + route.params.id,
		{
			method: 'POST',
		...
		.then((response: any) => {
			$toast.show({
				summary: 'Hotovo',
				detail: route.params.id === 'pridat' ? 'Rezervace byla vytvořena.' : 'Rezervace byla upravena.',
				severity: 'success',
			});
			if (!redirect && route.params.id === 'pridat') router.push('/ubytovani/rezervace/' + response.id);
			else if (redirect) router.push('/ubytovani/rezervace');
			else loadItem();
		})
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit rezervaci.', severity: 'error' });
		})
		.finally(() => { loading.value = false; });
}
			...
			<div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
				<div class="col-span-1 space-y-8 lg:col-span-9">
					...
					<LayoutContainer>
						...
						<div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-6">
							<BaseFormInput v-model="item.guest_firstname" label="Jméno" type="text" name="guest_firstname" rules="required" />
							<BaseFormInput v-model="item.guest_lastname" label="Příjmení" type="text" name="guest_lastname" rules="required" />
							<BaseFormInput v-model="item.guest_email" label="E-mail" type="email" name="guest_email" />
							<BaseFormInput v-model="item.guest_phone" label="Telefon" type="text" name="guest_phone" />
							<BaseFormInput v-model="item.number_of_guests" label="Počet hostů" type="number" name="number_of_guests" />
						</div>
					</LayoutContainer>
					...
					<LayoutContainer>
						...
						<div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-6">
							<BaseFormInput v-model="item.total_price" label="Celková cena" type="number" name="total_price" />
							<BaseFormSelect
							...
							/>
							<BaseFormTextarea v-model="item.notes" label="Poznámky" name="notes" class="sm:col-span-2" />
						</div>
					</LayoutContainer>
				</div>
