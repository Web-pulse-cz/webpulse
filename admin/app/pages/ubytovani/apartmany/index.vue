...
async function deleteItem(id: number) {
	...
	await client('/api/admin/apartment/' + id, {
		...
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se smazat apartmán.', severity: 'error' });
		})
	...
	<div>
		...
		<BaseTable
			...
			:columns="[
				...
				{ key: 'capacity', name: 'Kapacita', type: 'number', width: 80, hidden: false, sortable: false },
				{ key: 'base_price', name: 'Základní cena', type: 'number', width: 80, hidden: false, sortable: false },
				{ key: 'status', name: 'Stav', type: 'status', width: 80, hidden: false, sortable: false },
				{ key: 'position', name: 'Pořadí', type: 'number', width: 80, hidden: true, sortable: true },
			]"
