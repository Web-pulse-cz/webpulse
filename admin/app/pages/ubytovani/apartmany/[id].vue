...
async function loadLookups() {
	...
	await Promise.all([
		client('/api/admin/apartment/type', { method: 'GET', headers })
			.then((res: any) => {
				apartmentTypes.value = (Array.isArray(res) ? res : res.data || [])
					.map((x: any) => ({ value: x.id, name: x.name }));
			}),
		client('/api/admin/building', { method: 'GET', headers })
			.then((res: any) => {
				buildings.value = (Array.isArray(res) ? res : res.data || [])
					.map((x: any) => ({ value: x.id, name: x.name }));
			}),
		client('/api/admin/amenity', { method: 'GET', headers })
			.then((res: any) => {
				amenities.value = (Array.isArray(res) ? res : res.data || []);
			}),
		client('/api/admin/season', { method: 'GET', headers })
			.then((res: any) => {
...
async function loadItem() {
	...
	await client('/api/admin/apartment/' + route.params.id, {
		...
		.then((response: any) => {
			...
			breadcrumbs.value.push({ name: pageTitle.value, link: '/ubytovani/apartmany/' + route.params.id, current: true });
			fillEmptyTranslations();
			ensureAllSeasonPrices();
		})
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst apartmán.', severity: 'error' });
...
async function saveBlock() {
	if (!newBlock.value.start_date || !newBlock.value.end_date) {
		$toast.show({ summary: 'Upozornění', detail: 'Vyberte prosím datum od a do.', severity: 'warn' });
		return;
	}
	const client = useSanctumClient();
	...
	await client('/api/admin/apartment/block', {
		...
		.then(() => {
			...
			newBlock.value = { apartment_id: null, start_date: '', end_date: '', reason: 'maintenance', note: '' };
			loadBlocks();
		})
		.catch(() => {
...
async function saveItem(redirect = true as boolean) {
	...
	await client(
		isNew.value ? '/api/admin/apartment' : '/api/admin/apartment/' + route.params.id,
		{
			method: 'POST',
		...
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit apartmán.', severity: 'error' });
		})
		.finally(() => { loading.value = false; });
}
...
<template>
	<div class="space-y-6 pb-24">
		...
		<div v-if="!isNew" class="flex flex-wrap gap-2 border-b border-slate-200">
			...
			>
				{{ tab.name }}
				<span v-if="activeTab === tab.link" class="absolute bottom-0 left-0 h-0.5 w-full bg-indigo-600" />
			</button>
		</div>
				...
				<div class="col-span-1 space-y-8 lg:col-span-9">
					...
					<LayoutContainer>
						...
						<div class="grid grid-cols-1 gap-y-6 sm:grid-cols-3 sm:gap-x-6">
							<BaseFormSelect v-model="item.apartment_type_id" label="Typ apartmánu" name="apartment_type_id" :options="apartmentTypes" />
							<BaseFormSelect v-model="item.building_id" label="Budova" name="building_id" :options="buildings" />
							<BaseFormSelect
								v-model="item.status"
								label="Stav"
								name="status"
							...
							/>
							<BaseFormInput v-model="item.capacity" label="Kapacita (osob)" type="number" name="capacity" />
							<BaseFormInput v-model="item.bedrooms" label="Ložnice" type="number" name="bedrooms" />
							<BaseFormInput v-model="item.bathrooms" label="Koupelny" type="number" name="bathrooms" />
							...
							<BaseFormInput v-model="item.base_price" label="Základní cena / noc" type="number" name="base_price" />
							<BaseFormSelect
								v-model="item.currency_id"
								label="Měna"
								name="currency_id"
								:options="currencyStore.currencies?.map((c: any) => ({ value: c.id, name: c.code })) || []"
							/>
						</div>
					</LayoutContainer>
					...
					<LayoutContainer>
						...
						<div v-if="amenities.length > 0" class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
							<div
						...
						<p v-else class="text-center text-sm italic text-slate-400">
							Žádné vybavení zatím není v systému. Nejdřív si založ vybavení v sekci Ubytování → Vybavení.
						</p>
					</LayoutContainer>
					...
					<LayoutContainer>
						...
						<BaseFormUploadImage
							...
							@update-files="(files: any) => { item.image = files; }"
						/>
					</LayoutContainer>
					...
					>
						...
						<div class="flex items-center gap-2">
							<input
								...
								@input="setSeasonPrice(season.id, Number(($event.target as HTMLInputElement).value))"
							/>
			...
			<LayoutContainer>
				...
				<div class="mb-4 flex flex-wrap gap-2 border-b border-slate-200">
					<button
						...
						:class="reservationSubTab === tab.key ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900'"
						@click="switchReservationSubTab(tab.key as 'upcoming' | 'current' | 'past')"
					>
						{{ tab.label }}
						<span v-if="reservationSubTab === tab.key" class="absolute bottom-0 left-0 h-0.5 w-full bg-indigo-600" />
					</button>
				</div>

				<div v-if="reservations.length === 0" class="py-8 text-center text-sm italic text-slate-400">
			...
			<LayoutContainer>
				...
				<div class="mb-6 rounded-2xl bg-slate-50 p-6 ring-1 ring-slate-200">
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
						<BaseFormInput v-model="newBlock.start_date" label="Od" type="date" name="block_start" />
					...
					<BaseFormInput v-model="newBlock.note" label="Poznámka" type="text" name="block_note" class="mt-4" />
				</div>
				...
				<div v-else class="space-y-2">
					...
					>
						<div>
							<div class="font-medium text-slate-700">{{ block.start_date }} — {{ block.end_date }}</div>
							<div class="text-xs text-slate-500">
								{{ { maintenance: 'Údržba', owner: 'Pobyt majitele', other: 'Jiné' }[block.reason as string] }}
