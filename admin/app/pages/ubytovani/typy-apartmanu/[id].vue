...
const pageTitle = ref(route.params.id === 'pridat' ? 'Nový typ apartmánu' : 'Detail typu apartmánu');
...
async function saveItem(redirect = true as boolean) {
	...
	await client(
		route.params.id === 'pridat' ? '/api/admin/apartment/type' : '/api/admin/apartment/type/' + route.params.id,
		{
			method: 'POST',
		...
		.then((response: any) => {
			$toast.show({
				summary: 'Hotovo',
				detail: route.params.id === 'pridat' ? 'Typ apartmánu byl vytvořen.' : 'Typ apartmánu byl upraven.',
				severity: 'success',
			});
				...
				<div class="col-span-1 space-y-8 lg:col-span-9">
					...
					<LayoutContainer>
						...
						<BaseFormUploadImage
							...
							@update-files="(files: string[]) => { item.image = files; }"
						/>
					</LayoutContainer>
				</div>
