...
const pageTitle = ref('Vybavení');

const loading = ref(false);
const error = ref(false);

const breadcrumbs = ref([{ name: pageTitle.value, link: '/ubytovani/vybaveni', current: true }]);
...
async function loadItems() {
  ...
  await client('/api/admin/amenity', {
    method: 'GET',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se načíst vybavení.',
        severity: 'error',
      });
    })
    .finally(() => {
...
async function deleteItem(id: number) {
  ...
  await client('/api/admin/amenity/' + id, {
    method: 'DELETE',
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se smazat vybavení.',
        severity: 'error',
      });
    })
    .finally(() => {
...
<template>
  <div>
    <LayoutHeader
      ...
      :actions="[{ type: 'add', text: 'Přidat vybavení' }]"
      slug="amenities"
    />
    <BaseTable
      :items="items"
      :columns="[
        ...
        { key: 'icon', name: 'Ikona', type: 'text', width: 80, hidden: false, sortable: false },
