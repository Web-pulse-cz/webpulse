  ...
  await client('/api/admin/amenity/' + route.params.id, {
    ...
    .then((response: any) => {
      ...
      breadcrumbs.value.push({
        ...
        link: '/ubytovani/vybaveni/' + route.params.id,
        current: true,
      });
  ...
  await client(
    ...
    .then((response: any) => {
      $toast.show({
        summary: 'Hotovo',
        detail:
          route.params.id === 'pridat' ? 'Vybavení bylo vytvořeno.' : 'Vybavení bylo upraveno.',
        severity: 'success',
      });
      if (!redirect && route.params.id === 'pridat')
        router.push('/ubytovani/vybaveni/' + response.id);
    ...
    .catch(() => {
      ...
      $toast.show({
        summary: 'Chyba',
        detail: 'Nepodařilo se uložit vybavení.',
        severity: 'error',
      });
    })
    .finally(() => {
