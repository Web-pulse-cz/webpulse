...
const breadcrumbs = ref([{ name: pageTitle.value, link: '/ubytovani/rocni-obdobi', current: true }]);
...
async function loadItems() {
	...
	await client('/api/admin/season', {
		...
		.catch(() => {
			error.value = true;
			$toast.show({ summary: 'Chyba', detail: 'Nepodařilo se načíst roční období.', severity: 'error' });
		})
	...
	<div>
		...
		<BaseTable
			...
			:columns="[
				...
				{ key: 'is_recurring', name: 'Opakující se', type: 'status', width: 80, hidden: false, sortable: false },
