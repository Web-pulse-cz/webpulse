  ...
  await client<{
    ...
    .then((response) => {
      ...
      breadcrumbs.value.push({
        name: `Událost ${item.value.event.name} ─ ${item.value.firstname} ${item.value.lastname}`,
    ...
    .catch(() => {
      ...
      toast.add({
        title: 'Chyba',
        description: 'Nepodařilo se načíst registraci. Zkuste to prosím později.',
        color: 'red',
      });
    })
    .finally(() => {
  ...
  <div>
    <LayoutHeader
      :title="
        route.params.id === 'pridat'
          ? 'Nová registrace'
          : `Událost ${item.event.name} ─ ${item.firstname} ${item.lastname}`
      "
