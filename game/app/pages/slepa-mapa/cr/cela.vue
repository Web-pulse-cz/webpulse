...
// Omezení kategorií pouze na požadované tři
const categories = [
...
const processGeoData = (geoJson) => {
  ...
  features.forEach((item) => {
    ...
    if (g.type === 'Point') {
      ...
      const firstCoord = g.type === 'Polygon' ? g.coordinates[0][0] : g.coordinates[0][0][0];
      [lng, lat] = Array.isArray(firstCoord) && !Array.isArray(firstCoord[0]) ? firstCoord : firstCoord[0];
    }

    // Inteligentní popis na základě dostupných dat
    let infoText = 'Zajímavé místo v České republice.';
    if (p.ele) {
      infoText = `Nadmořská výška: ${p.ele} m n. m.`;
    } else if (p.population) {
      infoText = `Počet obyvatel: ${parseInt(p.population).toLocaleString()}`;
    } else if (p.wikipedia) {
      infoText = `Historická památka. Více na Wikipedii (heslo: ${p.wikipedia.split(':')[1]})`;
    }

    output.push({
      name: p.name || p["name:cs"] || 'Neznámé místo',
      lat,
      lng,
      info: infoText
    });
...
const handleMapClick = (e) => {
  ...
  const dist = calculateDistance(lat, lng, currentTarget.value.lat, currentTarget.value.lng);

  const pointsEarned = Math.max(0, 5000 - Math.round(dist * 100));
  ...
  // NAHRAZENÍ EMOJI ZA BAREVNÉ BODY
  const guessIcon = L.value.divIcon({
  ...
  const targetIcon = L.value.divIcon({
    className: 'custom-dot-container',
    html: `<div class="marker-dot target-dot"></div>`,
    iconSize: [20, 20],
    iconAnchor: [10, 10]
  });

  L.value.marker([lat, lng], { icon: guessIcon }).addTo(map.value);
  L.value.marker([currentTarget.value.lat, currentTarget.value.lng], { icon: targetIcon }).addTo(map.value);
  L.value.polyline([[lat, lng], [currentTarget.value.lat, currentTarget.value.lng]], {
...
const nextRound = () => {
  if (currentRound.value < totalRoundsInGame.value) {
    currentRound.value++;
    initRound();
  } else {
...
onMounted(async () => {
  if (import.meta.client) {
    ...
    // Zpracování dat pro tři hlavní kategorie
...
<template>
  <div class="flex h-screen w-screen overflow-hidden bg-slate-50 font-sans text-slate-900">
    ...
    <aside class="flex w-80 flex-col border-r border-slate-200 bg-white shadow-sm shrink-0 h-full">
      <div class="p-6 border-b border-slate-50">
        <div class="flex items-center gap-3 mb-1">
          <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-xl shadow-lg shadow-blue-100 font-bold text-white uppercase">M</div>
      ...
      <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
        ...
        <button
            ...
            :class="currentMode === cat.id ? 'bg-blue-50 text-blue-600 ring-1 ring-blue-100' : 'text-slate-500 hover:bg-slate-50'"
      ...
      <div class="p-6 bg-slate-50/50 border-t border-slate-100 mt-auto">
        <div class="grid grid-cols-2 gap-3 mb-4">
          <div class="rounded-2xl bg-white p-3 shadow-sm border border-slate-100">
            <span class="block text-[9px] font-bold uppercase text-slate-400 mb-1">Kolo</span>
            <span class="text-base font-bold text-slate-700">{{ currentRound }}<span class="text-slate-300 mx-1">/</span>{{ totalRoundsInGame }}</span>
          </div>
          <div class="rounded-2xl bg-white p-3 shadow-sm border border-slate-100">
            <span class="block text-[9px] font-bold uppercase text-slate-400 mb-1">Skóre</span>
            <span class="text-base font-bold text-blue-600">{{ (score || 0).toLocaleString() }}</span>
          </div>
        </div>
        <button @click="restartGame" class="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-900 py-3.5 text-sm font-bold text-white shadow-xl hover:bg-slate-800 active:scale-[0.98] transition-all">
          🔄 Restartovat hru
        </button>
      </div>
    </aside>
    ...
    <main class="relative flex flex-1 flex-col p-4 md:p-6 lg:p-8 min-w-0 h-full bg-[#f8fafc]">
      <div class="mb-6 flex items-center justify-between px-2 shrink-0">
        <div v-if="currentTarget" class="flex flex-col gap-1">
          <div class="flex items-center gap-2">
            <span class="px-2 py-0.5 rounded-md bg-blue-100 text-[10px] font-bold text-blue-600 uppercase">{{ categories.find(c => c.id === currentMode)?.name }}</span>
            <span class="h-1 w-1 rounded-full bg-slate-300"></span>
            <span class="text-xs font-medium text-slate-400 italic">Najdi vybrané místo</span>
          </div>
          <h2 class="text-4xl font-black text-slate-800 tracking-tight">
            Najdi: <span class="text-blue-600">{{ currentTarget.name }}</span>
          </h2>
        </div>
        <div v-else class="h-14 flex items-center text-slate-300 italic font-medium text-2xl">Žádná data v této kategorii...</div>
      </div>
      ...
      <div class="relative flex-1 overflow-hidden rounded-[2.5rem] border-[12px] border-white bg-white shadow-2xl shadow-blue-900/5 ring-1 ring-slate-200/50">
          ...
          <div v-if="lastGuess" class="absolute bottom-6 left-6 right-6 z-[1000] flex items-center justify-between rounded-3xl border border-white/40 bg-white/80 p-5 shadow-2xl backdrop-blur-xl ring-1 ring-black/5">
            ...
            <div class="flex items-center gap-6 border-l border-slate-200/50 pl-6 shrink-0">
              <div class="text-right">
                ...
                <div :class="lastGuess.points > 3500 ? 'text-green-600' : 'text-orange-500'" class="text-2xl font-black tabular-nums">
                  +{{ lastGuess.points.toLocaleString() }}
                </div>
                <p class="text-[10px] font-bold text-slate-400">Odchylka: {{ lastGuess.distance }} km</p>
              </div>
              <button @click="nextRound" class="rounded-2xl bg-blue-600 px-8 py-4 font-bold text-white shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all">
                Pokračovat
              </button>
            </div>
          </div>
        ...
        <div v-if="gameStatus === 'finished'" class="absolute inset-0 z-[2000] flex items-center justify-center bg-slate-900/40 backdrop-blur-md px-4 text-center">
          <div class="w-full max-w-sm rounded-[3rem] border border-slate-100 bg-white p-12 shadow-2xl">
            ...
            <div class="my-6">
              <p class="text-[10px] font-bold uppercase text-slate-400 mb-1 tracking-widest">Konečné skóre</p>
              <div class="text-7xl font-black text-slate-900 tracking-tighter">{{ score.toLocaleString() }}</div>
            </div>
            <button @click="restartGame" class="w-full rounded-2xl bg-blue-600 py-5 text-xl font-bold text-white shadow-2xl hover:bg-blue-700 active:scale-95 transition">Zkusit znovu</button>
          </div>
        </div>
      </div>
    </main>
...
.user-dot {
  background-color: #ef4444; /* Červená pro kliknutí uživatele */
}

.target-dot {
  background-color: #22c55e; /* Zelená pro správný cíl */
}
